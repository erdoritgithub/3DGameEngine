...
namespace Hazel {
	...
	static physx::PxFilterFlags HazelFilterShader(physx::PxFilterObjectAttributes attributes0, physx::PxFilterData filterData0, physx::PxFilterObjectAttributes attributes1, physx::PxFilterData filterData1, physx::PxPairFlags& pairFlags, const void* constantBlock, physx::PxU32 constantBlockSize)
	{
	...
	void Physics3D::Init()
	{
		HZ_CORE_ASSERT(!s_PXFoundation, "PhysXManager::Init shouldn't be called more than once!");

		s_PXFoundation = PxCreateFoundation(PX_PHYSICS_VERSION, s_PXAllocator, s_PXErrorCallback);
		HZ_CORE_ASSERT(s_PXFoundation, "PxCreateFoundation Failed!");
		...
		HZ_CORE_ASSERT(s_PXPhysicsFactory, "PxCreatePhysics Failed!");

		s_PXCookingFactory = PxCreateCooking(PX_PHYSICS_VERSION, *s_PXFoundation, s_PXPhysicsFactory->getTolerancesScale());
		HZ_CORE_ASSERT(s_PXCookingFactory, "PxCreatePhysics Failed!");
	}
